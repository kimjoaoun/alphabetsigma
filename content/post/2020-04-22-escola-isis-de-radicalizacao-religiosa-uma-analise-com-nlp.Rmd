...
Nesse post nós iremos fazer uma simples (e rápida) análise do dataset "ISIS Religious Texts v1", que pode ser encontrado [AQUI](https://www.kaggle.com/fifthtribe/isis-religious-texts). Essa base de dados contém textos de duas revistas do ISIS, estas sendo a "Dabiq" e a "Rumyiah", que são (ou eram?) usadas para propaganda política e recrutamento de novos membros para o grupo terrorista.

Primeiro começamos carregando as bibliotecas essenciais para a tarefa, com atenção à Rlang, que nos será bem util. Você pode conferir melhor suas funcionalidades nessa publicação [AQUI (em inglês)](https://www.tidyverse.org/blog/2018/03/rlang-0.2.0/), feita pelo time de desenvolvedores do Tidyverse.
...
Mas calma... Você, pessoa bonita e atenta, poderia me chamar atenção ao fato de que existe a função `tm::map(removePunctuation)`, que ao passarmos no [Corpus](https://en.wikipedia.org/wiki/Text_corpus) atinge o mesmo objetivo. Concordo, mas estava afim de brincar com o `rlang`, e também, a função também dá uma utilidade às [expressões regulares, que vimos no post anterior](https://alphabetsigma.netlify.app/2020/03/17/introducao-as-expressoes-regulares/).
...
Preste atenção ao fato de que a informação do nome da coluna não deve ser inserida como uma `'string'`, e sim como se fosse um `objeto`, [caso contrário, teremos um resultado inesperado](https://stackoverflow.com/questions/61377657/tidy-evaluation-not-working-with-mutate-and-stringr).
